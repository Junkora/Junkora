...
function updateProfileModal() {
  try {
    ...
    const curr = (typeof Mechanics !== 'undefined' && Mechanics.currencies) ? Mechanics.currencies : (Game.currencies || { cash: 0, junk: 0, sol: 0 });
    ...
    if (solEl) solEl.textContent = String(curr.sol ?? 0);
          ...
          try {
            ...
            btn.addEventListener('click', (e) => {
              ...
              const proceedSell = () => {
                ...
                // Add cash
                if (!Game.currencies) Game.currencies = { cash: (Game.coins ?? 0), junk: 0, sol: 0 };
  ...
  Object.entries(CROPS).forEach(([type, def]) => {
    ...
    btn.addEventListener('click', () => {
      ...
      if (cash >= def.seedPrice) {
        if (!Game.currencies) Game.currencies = { cash: cash ?? 0, junk: 0, sol: 0 };
...
function save() {
  try {
    const data = {
      ...
      currencies: Game.currencies ?? { cash: (Game.coins ?? 50), junk: 0, sol: 0 },
...
function load() {
  try {
    ...
    Game.currencies = data.currencies ?? Game.currencies ?? { cash: (data.coins ?? Game.coins ?? 50), junk: 0, sol: 0 };
